...
/* Modified version to work with struct Sndbuf_supernova. Modifications are needed in order to compile 
...
struct buffer_lock2
{
	buffer_lock2(const SndBuf_supernova * buf1, const SndBuf_supernova * buf2):
...
private:
	...
	const SndBuf_supernova * buf1_;
	const SndBuf_supernova * buf2_;
};

template <bool shared>
struct buffer_lock
{
	buffer_lock(const SndBuf_supernova * buf):
		buf_(buf)
	{
		if (!buf->isLocal) {
			if (shared)
